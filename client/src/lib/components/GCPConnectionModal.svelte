	...
	function handleSubmit() {
		...
		try {
			...
			if (!parsedKey.project_id || !parsedKey.private_key || !parsedKey.client_email) {
				throw new Error(
					'Missing required fields (project_id, private_key, client_email). Please use a valid Service Account Key.'
				);
			}
...
{#if isOpen}
	...
	>
		<div
			class="bg-bg-secondary w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden border border-border-primary"
			...
			<div class="p-6">
				<p class="text-text-secondary mb-4 text-sm">
					...
					<span class="text-xs opacity-75">
						Ensure the Service Account has <strong>Cloud Build Editor</strong> and
						<strong>Cloud Run Admin</strong> roles.
					</span>
				</p>
				...
				<form on:submit|preventDefault={handleSubmit}>
					...
					{#if error}
						<div
							class="mb-4 p-3 bg-red-900/20 border border-red-500/50 rounded-lg text-red-400 text-sm"
						>
							{error}
						</div>
					{/if}
